...
public class PlayerScriptGeral : MonoBehaviour
{
    ...
    [HideInInspector]
    public int vida;  //Rodrigo --> Variável de vida do player
    [HideInInspector]
    ...
    void Start(){
        ...
        {
            vida = _stats.StatHPVilao(vida);
            ataque = _stats.StatAtkVilao(ataque);
            mana = 0;
            equipe = '0';
        }
        else //Rodrigo --> senão, ele é um dos heróis
        {
            vida = _stats.StatHPHeroi(vida);
